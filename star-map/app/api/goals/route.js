...
import { createGoalFromSkill, toggleGoalStep, archiveGoal, deleteGoal, goalsSummaryForPrompt } from '@/lib/goals'
import { requireAuth, assertSameOrigin, readJsonBody } from '@/lib/auth'
import { trackReq } from '@/lib/track'

// 目标系统 v1（路线图 P2 提前落地）：
// GET  返回目标列表 + 活跃目标摘要（小星提醒/计划页共用）
// POST {action:'create', text}            手动创建：goalBreak 拆解 + 自动落计划栏目
//      {action:'toggleStep', goalId, stepIndex, done}  步骤完成/重开（manual 来源）
//      {action:'archive'|'delete', goalId}             归档/删除
export async function GET(req) {
  const auth = requireAuth(req)
  if (!auth.user) return auth.response
  const p = readProfile(auth.user.id)
  return Response.json({ goals: p.goals || [], summary: goalsSummaryForPrompt(p) })
}
...
export async function POST(req) {
  ...
  try {
    ...
    const res = await withStoreLock(() => {
      const p = readProfile(userId)
      let r
      if (action === 'toggleStep') r = toggleGoalStep(p, { goalId, stepIndex: Number(stepIndex), done: !!done })
      ...
      return { goal: r.goal || null }
    })
    if (res.error) return Response.json({ error: res.error }, { status: res.code })
    return Response.json({ ok: true, goal: res.goal })
  } catch (e) {
